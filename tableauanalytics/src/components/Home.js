...
import BlocIcon from "./BlocIcon";
import DailyActivity from "./DailyActivity";
import GraphPie from "./GraphPie";
import GraphRadar from "./GraphRadar";
import GraphSessions from "./GraphSessions";
 ...
 *
 * @param {Object} datas - The data object passed as props.
...
const Home = (datas) => {
  ...
  /**
   * Fetches the user information and sets the user name.
   * Executes when the `data` dependency changes.
   */
  useEffect(() => {
    if (data[0].userInfos) {
      setUserName(data[0].userInfos.firstName);
    }
  }, [data]);

  let radarDatas;
  data[0]["data"]
    ? (radarDatas = data[0]["data"])
    : (radarDatas = [
        { value: 0, kind: "Cardio" },
        { value: 0, kind: "Energie" },
        { value: 0, kind: "Endurance" },
        { value: 0, kind: "Force" },
        { value: 0, kind: "Vitesse" },
        { value: 0, kind: "Intensité" },
      ]);

  let macronutrientArray;
  data[0]["id"]
    ? (macronutrientArray = Object.values(data[0]["keyData"]))
    : (macronutrientArray = ["0Cal", "0g", "0g", "0g"]);

  const ArrayColor = ["#FBEAEA", "#E9F4FB ", "#FAF6E5 ", "#FBEAEF"];
  const ArrayIcons = [
    <svg
      width="35"
      height="35"
  ...
  return (
    <>
      ...
      <DailyActivity datas={data[0]["sessionsWeight"]} />

      <GraphSessions datas={data[0]["sessionsLength"]} />
      <GraphRadar datas={radarDatas} />
      <GraphPie datas={data[0]["todayScore"]} />
      <div className="icon-container">
        <BlocIcon
          value={macronutrientArray[0]}
          color={ArrayColor[0]}
          iconSvg={ArrayIcons[0]}
          nutrient={"Calories"}
        />
        <BlocIcon
          value={macronutrientArray[1]}
          color={ArrayColor[1]}
          iconSvg={ArrayIcons[1]}
          nutrient={"Proteines"}
        />
        <BlocIcon
          value={macronutrientArray[2]}
          color={ArrayColor[2]}
          iconSvg={ArrayIcons[2]}
          nutrient={"Glucides"}
        />
        <BlocIcon
          value={macronutrientArray[3]}
          color={ArrayColor[3]}
          iconSvg={ArrayIcons[3]}
          nutrient={"Lipides"}
        />
      </div>

      <Link to={`/user/12`} className="user1">
        Utilisateur 1
      </Link>
      <Link to={`/user/18`} className="user2">
        Utilisateur 2
      </Link>
    </>
  );
